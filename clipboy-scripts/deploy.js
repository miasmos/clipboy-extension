...
const zxpPath = './deploy/package.zip';
const notesPath = './deploy/release-notes.txt';
...
(async () => {
    try {
        ...
        try {
            await fsp.stat(path.resolve(zxpPath));
        } catch {
            console.error('Error: zxp does not exist in filesystem');
            process.exit(1);
        }
        const notesFile = await fsp.readFile(path.resolve(notesPath));
        ...
        if (notesFile) {
            ...
            if (name && notes.length > 0) {
                ...
                if (confirm) {
                    ...
                    await deployZxp(project, version, path.resolve(zxpPath));
                    console.log('Success');
                } else {
